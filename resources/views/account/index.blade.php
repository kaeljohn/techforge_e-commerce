        ...
        <div class="max-w-7xl mx-auto px-6 sm:px-12 lg:px-14">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-4 gap-10">
                ...
                <div class="lg:col-span-3 liquid-glass rounded-3xl p-6 sm:p-10 border border-white/10 shadow-2xl relative overflow-hidden min-h-[500px] transition-all duration-500 ease-in-out">
                    ...
                    <form action="{{ route('account.profile.update') }}" method="POST" class="flex flex-col-reverse md:flex-row gap-12 relative z-10">
                        @csrf
                        <!-- Form (Left side) -->
                    ...
                    <div id="pane-addresses" class="content-pane hidden">
                        <div class="border-b border-white/10 pb-4 mb-6 relative z-10 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                            ...
                            <button onclick="openModal('add-address-modal')" class="bg-primary hover:bg-[#ff8c33] text-white px-5 py-2.5 rounded-xl text-sm font-bold transition-all shadow-lg flex items-center gap-2 whitespace-nowrap">
                        ...
                        <div class="flex flex-col gap-4 mt-4">
                            <!-- Address 1 (Default) -->
                            <div class="bg-[#13131a] border border-primary/30 rounded-xl p-5 md:p-6 shadow-[0_4px_20px_rgba(255,107,0,0.05)] transition-all">
                                <div class="flex flex-col md:flex-row md:items-start justify-between gap-4">
                                    <div class="flex-1">
                                        ...
                                        <div class="flex items-center gap-2 mb-2">
                                            ...
                                            <span class="text-[10px] font-bold text-gray-400 border border-white/10 bg-white/5 px-2 py-0.5 rounded uppercase tracking-wider">Home</span>
                                        </div>
                                        <p class="text-sm text-gray-400 leading-relaxed max-w-2xl mt-3">
                                            123 TechForge Avenue, Silicon Valley Phase 1<br>
                                            Brgy. San Lorenzo, Makati City, Metro Manila, 1223
                                        </p>
                                    </div>
                                    <div class="flex items-center gap-3 md:flex-col md:items-end">
                                        <button class="text-primary hover:text-[#ff8c33] text-sm font-bold transition-colors">Edit</button>
                            ...
                            <!-- Address 2 -->
                            <div class="bg-[#1a1a1a] border border-white/5 hover:border-white/10 rounded-xl p-5 md:p-6 transition-all group">
                                <div class="flex flex-col md:flex-row md:items-start justify-between gap-4">
                                    <div class="flex-1">
                                        <div class="flex items-center gap-3 mb-2">
                                            <h3 class="text-lg font-bold text-white">{{ Auth::user()->name ?? 'John Doe' }}</h3>
                                        ...
                                        <p class="text-sm text-gray-400 leading-relaxed max-w-2xl mt-3">
                                            Tower 4, Enterprise Center, 6766 Ayala Avenue<br>
                                            Makati City, Metro Manila, 1200
                                        </p>
                                    </div>
                                    <div class="flex flex-col items-start gap-4 md:items-end">
                                        <div class="flex items-center gap-4">
                                            <button class="text-blue-400 hover:text-blue-300 text-sm font-bold transition-colors">Edit</button>
                                            <button class="text-red-400 hover:text-red-300 text-sm font-bold transition-colors">Delete</button>
                                        </div>
                                        <button class="text-gray-400 hover:text-white text-xs font-bold transition-colors border border-white/10 px-3 py-1.5 rounded-lg hover:border-white/30">Set as Default</button>
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <div id="add-address-modal" class="fixed inset-0 z-[100] hidden items-center justify-center">
                    ...
                    <div class="relative bg-[#13131a] border border-white/10 rounded-2xl p-6 md:p-8 w-full max-w-lg shadow-[0_0_50px_rgba(0,0,0,0.5)] transform scale-95 opacity-0 transition-all duration-300">
                        <div class="flex items-center justify-between mb-6">
                            ...
                            <button onclick="closeModal('add-address-modal')" class="text-gray-400 hover:text-white transition-colors">
                                <i class="ph-bold ph-x text-xl"></i>
                            </button>
                        </div>
                        <form>
                            <div class="flex flex-col gap-4">
                                <div class="grid grid-cols-2 gap-4">
                                    <div class="flex flex-col gap-2">
                                        ...
                                        <input type="text" placeholder="John Doe" class="w-full bg-black/40 border border-white/10 focus:border-primary focus:ring-1 focus:ring-primary focus:bg-black/60 rounded-xl px-4 py-3 text-sm text-white transition-all outline-none">
                                    </div>
                                    <div class="flex flex-col gap-2">
                                        <label class="text-xs font-bold text-gray-400 uppercase tracking-wider">Phone Number</label>
                                        <input type="text" placeholder="(+63) 912 345 6789" class="w-full bg-black/40 border border-white/10 focus:border-primary focus:ring-1 focus:ring-primary focus:bg-black/60 rounded-xl px-4 py-3 text-sm text-white transition-all outline-none font-mono">
                                    </div>
                                </div>
                                <div class="grid grid-cols-2 gap-4">
                                    <div class="flex flex-col gap-2">
                                        <label class="text-xs font-bold text-gray-400 uppercase tracking-wider">Region / Province</label>
                                        <input type="text" placeholder="Metro Manila" class="w-full bg-black/40 border border-white/10 focus:border-primary focus:ring-1 focus:ring-primary focus:bg-black/60 rounded-xl px-4 py-3 text-sm text-white transition-all outline-none">
                                    </div>
                                    <div class="flex flex-col gap-2">
                                        <label class="text-xs font-bold text-gray-400 uppercase tracking-wider">Postal Code</label>
                                        <input type="text" placeholder="1200" class="w-full bg-black/40 border border-white/10 focus:border-primary focus:ring-1 focus:ring-primary focus:bg-black/60 rounded-xl px-4 py-3 text-sm text-white transition-all outline-none font-mono">
                                    </div>
                                </div>
                                <div class="flex flex-col gap-2">
                                    <label class="text-xs font-bold text-gray-400 uppercase tracking-wider">Detailed Address</label>
                                    <input type="text" placeholder="House/Unit Number, Street, Barangay" class="w-full bg-black/40 border border-white/10 focus:border-primary focus:ring-1 focus:ring-primary focus:bg-black/60 rounded-xl px-4 py-3 text-sm text-white transition-all outline-none">
                                </div>
                                <div class="flex flex-col gap-2">
                                    <label class="text-xs font-bold text-gray-400 uppercase tracking-wider">Label</label>
                                    <div class="flex items-center gap-3">
                                        <label class="flex items-center gap-2 cursor-pointer bg-black/40 border border-white/10 rounded-xl px-4 py-2 hover:border-primary transition-colors">
                                            <input type="radio" name="address_label" value="home" class="accent-primary w-4 h-4" checked>
                                            <span class="text-sm text-white">Home</span>
                                        </label>
                                        <label class="flex items-center gap-2 cursor-pointer bg-black/40 border border-white/10 rounded-xl px-4 py-2 hover:border-primary transition-colors">
                                            <input type="radio" name="address_label" value="work" class="accent-primary w-4 h-4">
                                            <span class="text-sm text-white">Work</span>
                                        </label>
                                    </div>
                                </div>
                                <button type="button" class="mt-4 bg-gradient-to-r from-primary to-[#ff8c33] hover:from-[#ff8c33] hover:to-primary text-white w-full py-3 rounded-xl text-sm font-bold transition-all shadow-[0_0_15px_rgba(255,107,0,0.3)] hover:shadow-[0_0_25px_rgba(255,107,0,0.5)]" onclick="closeModal('add-address-modal'); showToast('Address saved successfully!')">
                                    Save Address
                                </button>
                            </div>
                        </form>
