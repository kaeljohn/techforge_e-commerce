...
@php
    ...
    // Determine the route parameters based on the type
    $routeParams = ['id' => $config->id];
    if ($type === 'custom') {
        $routeParams['type'] = 'custom';
    }
@endphp
...
<div class="config-card w-full liquid-glass rounded-2xl p-5 border border-white/10 flex flex-col group hover:border-primary/50 transition-all duration-300" data-platform="{{ $platform }}">
    ...
    <div class="mt-auto pt-4 border-t border-white/10 flex items-center justify-between min-h-[72px]">
        ...
        <a href="{{ route('build-overview', $routeParams) }}" class="w-10 h-10 rounded-xl bg-primary text-white flex items-center justify-center hover:bg-white hover:text-black hover:scale-110 transition-all shadow-[0_0_10px_rgba(255,107,0,0.4)] shrink-0">
