...
class PcPartController extends Controller
{
    public function index(Request $request)
    {
        ...
        $collections = $collections->concat(\App\Models\PcCase::all()->map(function($i) { $i->category = 'Case'; $i->filter_key = 'cases'; return $i; }));
        $collections = $collections->concat(\App\Models\ChasisFan::all()->map(function($i) { $i->category = 'Chasis Fan'; $i->filter_key = 'chasis-fans'; return $i; }));
        $collections = $collections->concat(\App\Models\Storage::all()->map(function($i) { $i->category = 'Storage'; $i->filter_key = 'storages'; return $i; }));
        $collections = $collections->concat(\App\Models\Gpu::all()->map(function($i) { $i->category = 'Video Card'; $i->filter_key = 'video-cards'; return $i; }));
        $collections = $collections->concat(\App\Models\Cpu::all()->map(function($i) { $i->category = 'Processor'; $i->filter_key = 'processors'; return $i; }));
        $collections = $collections->concat(\App\Models\Cooler::all()->map(function($i) { $i->category = 'Cooler'; $i->filter_key = 'coolers'; return $i; }));
        $collections = $collections->concat(\App\Models\Motherboard::all()->map(function($i) { $i->category = 'Motherboard'; $i->filter_key = 'motherboards'; return $i; }));
        $collections = $collections->concat(\App\Models\Ram::all()->map(function($i) { $i->category = 'RAM'; $i->filter_key = 'rams'; return $i; }));
        ...
        return view('store.pc-parts', compact('items', 'initialCategory'));
    }
}
